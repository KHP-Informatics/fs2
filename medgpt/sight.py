...
import numpy as np
import torch
from medgpt.utils.cdb_utils import get_parents_map, get_children_map, get_siblings_map
...
class Sight(object):
    ...
    def _predict(self, stream, create_position_ids=False, skip_oov=False, input_ids=None):
        self.model.eval()
        if input_ids is None:
            _stream = self.tokenizer(stream, add_special_tokens=False, return_tensors='pt').to(self.device)
        else:
            _stream = {'input_ids': torch.tensor([input_ids]).to(self.device), 
    ...
    def next_concepts(self, stream=None, type_ids=None, n=5, p_new=True, p_old=False, create_position_ids=False, prediction_filters=[], cui_filter=None,
                      skip_oov=False, token_type2tokens=None, tkn2id=None, id2token=None, input_ids=None):
        r'''
        ...
        if token_type2tokens is None:
            token_type2tokens = self.tokenizer.token_type2tokens
        if tkn2id is None:
            tkn2id = self.tokenizer.tkn2id
        
        if input_ids is None:
            input_ids = self.tokenizer(stream, add_special_tokens=False)['input_ids']
        ...
        if type_ids is not None:
            ...
            select_tokens = None
        ps = self._predict(stream, create_position_ids=create_position_ids, input_ids=input_ids)
        preds = np.argsort(-1 * ps)
        candidates = []
        sep_ids = [tkn2id[x] for x in token_type2tokens['sep']]
        ...
        def update_ignore_cuis(ignore_cuis, prediction_filters, cui):
            ...
            return ignore_cuis
        if stream is None:
            stream = self.tokenizer.convert_ids_to_tokens(input_ids)
        for cui in stream:
