...
def metrics_data2df(metrics_data, tkn2name=None, main='positives', temporality='all'):
    ...
    out = sorted([(
        ...
        "{:.2f}".format(d['fn_positives'][temporality].get(cui, 0) / 
                        (d['fn_positives'][temporality].get(cui, 0) + d['fn_negatives'][temporality].get(cui, 1) )), # I know the default 1 could reduce scores but it is fine.
...
def precision(predictions, label_ids, id2tkn, token_type2tokens, type_data, select_token_types={'T-11'}, prediction_scope='one', shifted_labels=False,
    ...
    if old_data:
        ...
        overall_correct = old_data['overall_correct']
        overall_incorrect = old_data['overall_incorrect']
    ...
    for ind, lbl in enumerate(label_ids):
        ...
        # Have an overall score for correct/incorrect predictions of everything
        for one_ind, one_lbl in enumerate(lbl):
            if one_lbl == outputs[ind][one_ind-label_position_shift][0]:
                overall_correct += 1
            else:
                overall_incorrect += 1
            ...
            for i in range(start, len(lbl)):
                if lbl[i] != -100:
                    ...
                    if type_label in select_token_types and enough_time_left:
                        candidates = []
                        select_tokens = token_type2tokens[type_label[-4:]] # Take -4: as we only want the last 4 chars from the type, because of risk-T-11
                        if predictions_are_scores:
                        ...
                        if not candidates:
                            print(ind, lbl, tkn_label, type_label, is_new_label)
                        ...
                        if fn_lbl[i] == 1:
                            ...
                            count_tkn_candidate(positives=fn_negatives, negatives=fn_positives,
                                                tkn_candidate=tkn_label, temporality=temporality)
                        elif fn_lbl[i] == 0:
                            tp_for_fn['all'] += 1
    ...
    def __call__(self, p, metrics_data=None):
        ...
        if self.return_all_metrics:
            ...
            return {
                ...
                'overall_correct': metrics_data['overall_correct'],
                'overall_incorrect': metrics_data['overall_incorrect'],
            }
